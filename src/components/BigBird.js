...
function BigBird() {
	...
	useEffect(() => {
		...
		const birdNumber = location.state?.birdNum ?? ('A00000' + randomBird);
		const url = process.env.REACT_APP_DATA_SINGLE_URL + '?serviceKey=' + process.env.REACT_APP_DATA_KEY + '&q1=' + birdNumber;
		async function fetchBird() {
			try {
				...
				const imgUrl = 'https://joryudogam.s3.ap-northeast-2.amazonaws.com/birds/' + birdNumber + '.jpeg';
